...
from helpers.getWeaponsFromCollection import getWeaponsFromCollection
from helpers.findIds import findIds
...
def calc_ev(cost, avgfloat, inputcoll, inputgrade,statty=False):
    outputs = getWeaponsFromCollection(inputcoll,weapon_grades[weapon_grades.index(inputgrade)+1])
    wear = []
    check_over = []
    over_floats = []
    ids = []
    ...
    if outputs_check != str(wear) or stat_check != "Stat =" + str(statty):
        buffApiCaller = Buff(
